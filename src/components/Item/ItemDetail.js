...
import { logos } from "./ItemDetailStrings";


export const ItemDetail = ({ itemSelect }) => {

    let { name, model, details, price, url, img, logo, mark, year, version, condition, stock, description } = itemSelect
    ...
    return (
        <div className="absolute flex w-full h-screen top-0 pt-16 justify-center" >
            <div className="flex w-11/12 h-auto justify-around px-6 py-8 justify-center bg-gradient-to-t from-zinc-200 to-zinc-100">
                ...
                <div className="flex  flex-col items-center w-1/2">
                    <img src={bigImg} alt={name} className="h-full object-contain" />
                </div>
                <div className="flex flex-col items-end w-1/4 justify-items-center text-right">
                    <img src={logos[logo]} alt={logo} className="h-16 w-full mt-2 mb-6 object-contain justify-self-end" />
                    <h2 className="font-fredoka pr-1 text-md bg-gradient-to-r from-transparent to-amarillo">{name} {model}</h2>
                    <h3 className="font-racing pr-1 text-4xl my-1">{details}</h3>
                    ...
                    <div className="flex flex-col items-end my-2">
                        ...
                        <ItemCondition cond={itemCondition}>{itemCondition}</ItemCondition>
