...
const ItemCount = (props) => {
    ...
    const addToCart = () => {
        if ((props.count > 0) && (realStock >= 1) && (showStock >= 0)) {
            ...
            setRealStock(realStock -= props.count)
            setStock(showStock -= 1)
            props.setCount(1)
    ...
    return (
        <div className="flex flex-col w-48 h-28 bg-white rounded-lg my-4 font-ptsans m-auto">
            <div className="w-full h-2/5 bg-black text-white text-center text-sm py-1 font-roboto rounded-supboxes antialiased">{Product}{showStock}{showStock === 1 ? ' Unidad' : Units}{showStock > 0 ? " más" : ""}</div>
