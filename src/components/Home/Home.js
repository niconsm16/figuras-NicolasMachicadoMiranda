...
const Home = () => {
    ...
    const cMotionLegend = `absolute top-[20%] mx-12 z-30 w-1/3 ${(count % 2) === 0 ? 'left-0 text-left' : 'right-0 text-right'}`
    const cLegend = 'flex place-self-center grid text-white font-futura text-6xl transition-opacity'
    const cImg = "w-full object-cover h-[60vh] blend"
